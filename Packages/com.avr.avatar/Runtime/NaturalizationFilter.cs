...
public class NaturalizationFilter
{
    ...
    public NaturalizationFilter(int point_amount, int k) {
        refpoints = refpoints.Take(Mathf.Min(refpoints.Count, point_amount)).ToList();
        this.k = k;
    }
    ...
    public Vector3 naturalize_point(Vector3 wpos) {
        ...
        Vector3 targetPos = AVR.Core.AVR_PlayerRig.Instance.MainCamera.transform.TransformPoint(closest);
